...
const Login = () => {
  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-screen bg-gray-50 px-2'>
      ...
      <div className="sm:w-[500px] w-full border border-gray-300/60 rounded-2xl px-8 bg-white relative overflow-hidden">
        {state === "forgot-password" ? (
          <ForgotPassword onBack={() => setState("login")} />
        ) : (
          <form onSubmit={handleSubmit} className="text-center">
            ...
            <p onClick={() => setState(prev => prev === "login" ? "register" : "login")} className="text-gray-500 text-sm mt-4 mb-11">
              {state === "login" ? "Don't have an account?" : "Already have an account?"}
              <span className="text-green-500 hover:underline cursor-pointer ml-1 font-medium">click here</span>
            </p>
          </form>
        )}
      </div>
